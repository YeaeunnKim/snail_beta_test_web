...
## 0. 자동 배포 (기본 경로 — GitHub Actions)

**`main`에 머지되면 GitHub Actions 가 자동으로 Vercel 프로덕션에 배포한다.**
워크플로: [.github/workflows/deploy.yml](.github/workflows/deploy.yml)

Vercel 네이티브 Git 연동은 팀 **author 체크("Deployment Blocked", §6 참고)** 때문에 못 붙는다
(이 repo 소유자가 `snail8` 팀 멤버가 아님). 그래서 Actions 가 **Vercel CLI 로 prebuilt 배포**한다 —
토큰 소유자(팀 멤버)에게 귀속되고 Git author 컨텍스트가 없어 체크를 우회한다.

**최초 1회 세팅 (repo 관리자):**
1. 토큰 발급(사장님/glamnowlab): https://vercel.com/account/tokens → **Scope=snail8**, 만료일 지정 권장
2. Secret 등록: repo → Settings → Secrets and variables → Actions → `VERCEL_TOKEN` = 발급한 토큰
   (CLI: `gh secret set VERCEL_TOKEN --repo YeaeunnKim/snail_beta_test_web`)

> Secret 이 없으면 워크플로는 실패 대신 **조용히 skip**(경고만). 등록 후 다음 머지부터 자동 배포.
> `VERCEL_TOKEN` 은 `snail8` 팀 전체 배포권이므로 만료 짧게 + 담당 변경 시 Revoke.

---

## 1. 수동 배포 (fallback — 고정 main 배포 스크립트)

Actions 가 막히거나 긴급 배포가 필요할 때 쓰는 fallback. `scripts/deploy-main.ps1`이
GitHub 원격 `main`을 임시 폴더에 shallow clone하고, Vercel org/project id를 고정한 뒤
`snail8/snail_beta_test_web`으로 배포한다(작업자 로컬 트리/브랜치와 무관).

| 트리거 | 결과 | URL |
|---|---|---|
| `pnpm deploy:main` | **Production 배포** | `https://snailbetatestweb.vercel.app` |
