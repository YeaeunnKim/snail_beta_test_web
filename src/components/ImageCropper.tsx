...
function clampRect(r: CropRect, bounds: { w: number; h: number }): CropRect {
  const w = Math.max(MIN_SIZE, Math.min(r.w, bounds.w));
  const h = Math.max(MIN_SIZE, Math.min(r.h, bounds.h));
...
export function ImageCropper({ file, onCropped, onCancel, onSkip, aspect, title }: ImageCropperProps) {
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (drag.mode === 'move') {
      ...
      const w = drag.startRect.w + dx;
      const h = aspect ? w / aspect : drag.startRect.h + dy;
