 ...
 *
 * 모델(when2meet + 주간 반복): 기본은 회색(예약 불가). 드래그로 하얗게 칠한 시간 = 예약 가능.
 *   - 요일 근무창 = 그 요일 모든 날짜에서 켠 시간의 합집합 외곽 → 디자이너 주간 스케줄(ScheduleEntry)
 *   - 어떤 날짜에서 근무창 안을 "일부만" 켠 경우, 안 켠 연속 구간 → 그 날짜 휴무(TimeOff)
 *   - 그 요일을 어떤 날짜에서도 하나도 안 켰으면 → 요일 휴무일(is_day_off)
 *   - ⚠️ 특정 날짜에서 "한 슬롯도 안 켰으면" 미편집으로 보고 요일 근무창을 그대로 상속(가능 유지).
 *     → 한 주만 칠해도 같은 요일의 나머지 주가 예약 가능으로 유지된다(주간 반복 패턴).
 ...
 *   - 근무창 안을 "일부만" 켠 날짜: 안 켠 연속 구간 → 그 날짜 휴무(TimeOff).
 *   - 근무창 안에 한 슬롯도 안 켠 날짜: 미편집으로 보고 근무창을 상속(휴무 없음) → 주간 반복.
...
export function availToWeeklyScheduleAndTimeOff(
  ...
  // 3) 날짜별 휴무: 요일 근무창 안에서 그날 안 켠 연속 구간 → 부분 휴무 블록.
  //    ⚠️ 주간 반복 패턴: "한 슬롯도 안 켠 날짜"는 편집하지 않은 날로 보고 요일 근무창을
  //    그대로 상속시킨다(휴무 만들지 않음). 이렇게 해야 한 주만 칠해도 나머지 주가
  //    같은 요일 시간을 따라 예약 가능으로 유지된다(안 그러면 안 칠한 모든 날이 종일 휴무가 됨).
  //    특정 날짜만 통째로 쉬려면 이 화면 드래그가 아니라 별도 휴무 지정이 필요하다.
  const timeOffs: TimeOffCreate[] = [];
  for (const date of dates) {
    const w = windowByWeekday.get(appWeekday(date));
    if (!w) continue; // 요일 자체가 휴무 → 근무창 없음, 별도 휴무 블록 불필요
    // 그날 근무창 안에 켠 슬롯이 하나도 없으면 → 미편집으로 간주해 요일 패턴 상속(휴무 없음).
    let anyOn = false;
